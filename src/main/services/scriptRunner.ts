 ...
 * containment and a minimal API, not as a hard security boundary. If Rewind
...
export function runScript(
  ...
  const sandbox = {
    ...
    vars: Object.freeze({
      ...
      get: (name: unknown) =>
        typeof name === "string" ? result.varsSet[name] : undefined,
    }),
      ...
      result.assertions.push({
        expr:
          typeof message === "string"
            ? message
            : `assertion ${result.assertions.length + 1}`,
        pass: Boolean(cond),
