...
interface PmV1Request {
  ...
  rawModeData?: unknown;
  data?:
    | { key?: string; value?: string; enabled?: boolean; type?: string }[]
    | string;
...
function applyAuth(
  ...
  switch (auth.type) {
    ...
    default:
      warnings.push(
        `“${node.name}”: auth type "${auth.type}" isn't supported — left as inherit.`,
      );
  }
}
...
function convertV2Request(
  ...
  if (!METHODS.includes(method)) {
    stats.warnings.push(
      `“${name}”: method ${method} isn't supported — skipped.`,
    );
    return null;
  }
...
function convertV1Request(
  ...
  if (pm.dataMode === "raw") {
    ...
    node.body = {
      mode: "json",
      text: JSON.stringify(
        { query: pm.graphqlModeData.query, variables: {} },
        null,
        2,
      ),
    };
  ...
  if (pm.tests?.trim())
    node.scripts.postResponse = commentedScript(
      pm.tests.split("\n"),
      "test script",
    );
...
function convertEnvironment(
  ...
  return {
    ...
    variables: (pm.values ?? [])
      .filter((v) => v.key)
      .map((v) =>
        kv(v.key!, mapVars(String(v.value ?? "")), v.enabled !== false),
      ),
  };
}
...
export function convertPostman(raw: string): PostmanConversion {
  ...
  if (!doc || typeof doc !== "object")
    throw new Error("File is not a Postman export.");
  ...
  if (isV2Collection(doc)) {
    ...
    environments.push(
      convertEnvironment(doc as PmEnvironment, "Postman environment"),
    );
  } else if (
    ...
    for (const c of dump.collections ?? []) {
      ...
      if (isV2Collection(col))
        collections.push(convertV2Collection(col as PmV2Collection, stats));
    ...
    if (dump.globals?.length) {
      environments.push(
        convertEnvironment(
          { name: "Postman globals", values: dump.globals },
          "Postman globals",
        ),
      );
    }
