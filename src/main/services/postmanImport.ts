...
interface PmV2Collection {
  info?: { name?: string; schema?: string; version?: string | { major?: number } }
  item?: PmV2Item[]
  variable?: PmVariable[]
  auth?: PmAuth
}
...
/** Postman and Relay share `{{var}}` syntax; only dynamic variables need mapping. */
...
/** `/users/:id` → `/users/{{id}}` (Postman path variables → Relay variables). */
...
/** Postman scripts use the pm.* API, which Relay's sandbox doesn't provide — keep them, commented. */
...
function convertV2Request(item: PmV2Item, stats: ConvertStats): RequestNode | null {
  ...
  if (body?.mode === 'raw' && typeof body.raw === 'string' && body.raw.trim()) {
    ...
    node.body = { mode: 'json', text: JSON.stringify({ query: body.graphql.query, variables }, null, 2) }
    if (!hasHeader(node.headers, 'Content-Type')) node.headers.unshift(kv('Content-Type', 'application/json'))
  }
  ...
  const testExec = (item.event ?? [])
    ...
    .flatMap((ev) => (Array.isArray(ev.script!.exec) ? ev.script!.exec! : [ev.script!.exec as string]))
...
function convertV2Collection(pm: PmV2Collection, stats: ConvertStats): Collection {
  ...
  // Collection-level bearer token → `token` variable, which Relay's "inherit" auth picks up.
  if (pm.auth?.type === 'bearer') {
    const token = authParam(pm.auth.bearer, 'token')
    if (token && !variables.some((v) => v.key === 'token')) variables.push(kv('token', mapVars(token)))
  }
  return {
    id: newId(),
...
function convertV1Request(pm: PmV1Request, stats: ConvertStats): RequestNode | null {
  ...
  if (pm.headerData?.length) {
    for (const h of pm.headerData) if (h.key) headers.push(kv(h.key, mapVars(h.value ?? ''), !h.disabled))
  ...
  if (pm.tests?.trim()) node.scripts.postResponse = commentedScript(pm.tests.split('\n'), 'test script')
...
function convertV1Collection(pm: PmV1Collection, stats: ConvertStats): Collection {
  ...
  for (const f of pm.folders ?? []) {
    if (!f.id) continue
    foldersById.set(f.id, { id: newId(), type: 'folder', name: f.name?.trim() || 'Folder', children: [] })
  }
...
function isEnvironment(doc: Record<string, unknown>): boolean {
  ...
  return scope === 'environment' || scope === 'globals' || (typeof doc.name === 'string' && !doc.requests && !doc.item)
}
...
export function convertPostman(raw: string): PostmanConversion {
  ...
  try {
    ...
    throw new Error('File is not valid JSON. Export from Postman as JSON (Collection v2.x, environment, or data dump).')
  }
  ...
  if (isV2Collection(doc)) {
    ...
    for (const c of dump.collections ?? []) {
      ...
      else if (isV1Collection(col)) collections.push(convertV1Collection(col as PmV1Collection, stats))
    }
    for (const env of dump.environments ?? []) environments.push(convertEnvironment(env, 'Postman environment'))
    if (dump.globals?.length) {
      environments.push(convertEnvironment({ name: 'Postman globals', values: dump.globals }, 'Postman globals'))
    }
  } else if (isV1Collection(doc)) {
    collections.push(convertV1Collection(doc as PmV1Collection, stats))
  } else {
    throw new Error('Not a Postman export (expected a Collection v2.x, an environment, or a data dump).')
  }

  if (!collections.length && !environments.length) {
    throw new Error('Nothing importable found in this file.')
  }
  return { collections, environments, requestCount: stats.requestCount, warnings: stats.warnings }
}
