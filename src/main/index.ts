...
// The app used to be "Rewind" — carry its data directory across the rename.
// userData is pinned explicitly: setName() alone doesn't re-derive it this early.
{
  ...
  const relayDir = path.join(appData, "Rewind");
  ...
  app.whenReady().then(async () => {
    ...
    registerIpc((next) => {
      for (const win of BrowserWindow.getAllWindows())
        win.setBackgroundColor(chromeBg(next.theme));
    });
    await createWindow(settings);

    app.on("activate", async () => {
      if (BrowserWindow.getAllWindows().length === 0)
        await createWindow(await getSettings());
    });
  });
