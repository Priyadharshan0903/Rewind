...
export function registerIpc(onThemeChange: (settings: Settings) => void): void {
  ...
  ipcMain.handle(IPC.collectionSave, (_e, collection: Collection) =>
  ...
  ipcMain.handle(
    IPC.collectionExport,
    async (e, collectionId: string): Promise<ExportResult> => {
  ...
  ipcMain.handle(IPC.envSave, (_e, envs: Environment[]) =>
  ...
  ipcMain.handle(
    IPC.httpSend,
    async (_e, payload: SendPayload): Promise<Run> => {
      const [workspace, environments, settings, collections] =
        await Promise.all([
          getWorkspace(),
          getEnvironments(),
          getSettings(),
      ...
      const env =
        environments.find((x) => x.id === workspace.activeEnvironmentId) ??
        environments[0];
      ...
      for (const h of req.headers) {
        ...
        headers.push([
          interpolate(h.key, vars).text,
          interpolate(h.value, vars).text,
        ]);
      }
      ...
      if (!hasAuthHeader) {
        if (req.auth.mode === "inherit" && vars.token)
          ...
          headers.unshift([
            "Authorization",
            `Bearer ${interpolate(req.auth.token, vars).text}`,
          ]);
      ...
      const fields = (req.body.form ?? []).filter(
      ...
      const result = await sendHttp(
        payload.sendId,
        resolved,
        settings.responseBodyLimitBytes,
      );
        ...
        if (entries.length && env) {
          for (const [key, value] of entries) {
            ...
            if (existing) existing.value = value;
              ...
              env.variables.push({ id: newId(6), key, value, enabled: true });
          }
  ...
  ipcMain.handle(
    IPC.runsSaveExample,
    async (_e, runId: string): Promise<Collection | null> => {
  ...
  ipcMain.handle(IPC.transferImport, async (e): Promise<ImportResult> => {
    ...
    if (bundle?.format !== "rewind-bundle" || bundle.version !== 1) {
      return {
        error:
          'Not a Rewind bundle (expected format "rewind-bundle", version 1).',
      };
    }
    if (
      !bundle.workspace ||
  ...
  ipcMain.handle(IPC.openapiImport, async (e): Promise<OpenApiImportResult> => {
    ...
    try {
      ...
      const { collection, requestCount, folderCount } =
        convertToCollection(doc);
      await saveCollection(collection);
      return {
        collection,
