...
export const useRuns = create<RunsState>((set, get) => ({
  ...
  loadForRequest: async (requestId) => {
    const panelRuns = await window.relay.listRuns({ requestId })
    const currentRun = panelRuns[0] ? await window.relay.getRun(panelRuns[0].id) : null
    const compareId = panelRuns[1]?.id ?? null
    const compareRun = compareId ? await window.relay.getRun(compareId) : null
  ...
  send: async () => {
    ...
    try {
      ...
      set((s) => ({
        ...
        compareRun:
          s.compareRun ?? (s.currentRun && s.currentRun.requestId === run.requestId ? s.currentRun : null)
      }))
    } catch (err) {
      set({ sending: false, sendId: null, sendError: err instanceof Error ? err.message : String(err) })
    }
  },

  cancelSend: () => {
    const { sendId } = get()
    if (sendId) void window.relay.cancel(sendId)
  },
  ...
  setCompare: async (id) => {
    ...
    set({ compareId: id })
    const compareRun = await window.relay.getRun(id)
    set({ compareRun })
  },

  loadAll: async () => {
    const { filterStatus, filterMethod } = get()
    const allRuns = await window.relay.listRuns({ statusClass: filterStatus, method: filterMethod })
    ...
    if (histSelectedId && get().histDetail?.id !== histSelectedId) {
      set({ histDetail: await window.relay.getRun(histSelectedId) })
  ...
  selectHist: async (id) => {
    ...
    set({ histDetail: await window.relay.getRun(id) })
  },
  ...
  handleAppended: (summary) => {
    ...
    set((s) => ({
      ...
      panelRuns:
        app.selection?.requestId === summary.requestId ? dedupe([summary, ...s.panelRuns]) : s.panelRuns
    }))
  }
}))
