...
export const useApp = create<AppState>((set, get) => ({
  ...
  replaceCollection: (collection) =>
    set({
      collections: get().collections.map((c) => (c.id === collection.id ? collection : c))
    }),
  ...
  adoptCollection: (collection) => {
    set({
      collections: [...get().collections.filter((c) => c.id !== collection.id), collection]
    })
...
function mutateCollection(
  ...
  set({
    collections: get().collections.map((c) => (c.id === collectionId ? fn(c) : c))
  })
...
export function mergedVars(state: AppState, collectionId?: string | null): Record<string, string> {
  ...
  const env =
    state.environments.find((e) => e.id === state.workspace?.activeEnvironmentId) ??
