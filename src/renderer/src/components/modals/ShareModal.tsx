...
export function ShareModal(): React.JSX.Element {
  ...
  const invite = (): void => {
    toast('Invites need a Relay account — Relay is local-first for now')
    setEmail('')
  }
  ...
  const doExport = async (): Promise<void> => {
    setBusy(true)
    try {
      const result = await window.relay.exportBundle({ includeHistory })
    ...
    <Overlay onClose={toggleShare} center>
      <div className="modal share-modal">
        ...
        <div className="invite-empty">
          No teammates yet — invites need a Relay account. Your workspace stays on this machine until you share it.
        </div>
        <div className="move-section">
          <span className="micro-label">MOVE TO ANOTHER DEVICE</span>
          <span className="move-note">
            Bundle collections, environments and run history into one plain-JSON file — import it on any machine, no
            account needed.
          </span>
        ...
        <div className="modal-footer-note">
          ...
          Invites need a Relay account · exports stay fully local
        </div>
      </div>
    </Overlay>
  )
