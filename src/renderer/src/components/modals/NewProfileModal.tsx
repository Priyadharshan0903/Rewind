...
export function NewProfileModal(): React.JSX.Element {
  ...
  const create = async (): Promise<void> => {
    ...
    setBusy(true)
    try {
      const result = await window.relay.createProfile(name.trim())
    ...
    <Overlay onClose={() => closeOverlays()} center>
      <div className="modal new-profile-modal">
        ...
        <span className="move-note">
          A profile is a separate local workspace — its own collections, environments and run history, stored side by
          side on this machine.
        </span>
