  ...
  if (!collections.length) {
    return (
      <div className="docs-view">
        <div className="docs-empty">No collections yet — create one in the Runbook to generate docs.</div>
      </div>
    )
  }
...
function DocsNav({
  ...
  return (
    <aside className="docs-nav">
      ...
      <select className="docs-select" value={collectionId ?? ''} onChange={(e) => onPick(e.target.value)}>
        {collections.map((c) => (
...
function DocsContent({ docs, loading }: { docs: CollectionDocs; loading: boolean }): React.JSX.Element {
  ...
  return (
    <article id={`ep-${ep.requestId}`} className="docs-ep">
      <div className="docs-ep-head">
        ...
        {ep.runCount > 0 && <span className="docs-runs">{ep.runCount} run{ep.runCount > 1 ? 's' : ''}</span>}
      </div>
          ...
          {ep.params.map((p, i) => (
            <div key={i} className="docs-param-row">
              ...
              <span className="docs-param-ex">
                {p.example != null && p.example !== '' ? <code>{p.example}</code> : <span className="docs-muted">—</span>}
              </span>
            </div>
          ))}
        </div>
      ...
      {ex ? (
        <div className="docs-example">
          ...
          <div className="docs-ep-actions">
            <CopyMenu
              req={{ method: ex.reqMethod, url: ex.reqUrl, headers: ex.reqHeaders, bodyText: ex.reqBody }}
            />
          </div>
        </div>
      ) : (
        <div className="docs-noex">No example captured yet — send this request once and it appears here.</div>
      )}
    </article>
  )
}
