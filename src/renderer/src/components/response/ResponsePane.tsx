...
import { Search, PanelRight, Bookmark, RotateCw, Check, X } from 'lucide-react'
        ...
        {run?.script && !sending && (
          ...
          >
            {run.script.error ? (
              ...
              <>
                <Check size={12} strokeWidth={2.6} /> {run.script.assertions.length || 'script'} ok
              </>
            )}
          </span>
        )}
        {run?.error && !sending && (
          <button className="text-btn retry-btn copy-trigger" onClick={() => void send()}>
