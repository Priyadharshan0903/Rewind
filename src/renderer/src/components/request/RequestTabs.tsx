...
export function RequestTabs({ request }: { request: RequestNode }): React.JSX.Element {
  ...
  const startDrag = (e: React.PointerEvent): void => {
    ...
    const onMove = (ev: PointerEvent): void => {
      latest = Math.max(
        90,
        Math.min(startH + (ev.clientY - startY), Math.round(window.innerHeight * 0.65))
      )
      if (!frame) {
  ...
  return (
    <div className="headers-grid">
      ...
      <button
        ...
        onClick={() =>
          commit([...fields, { id: newId(6), key: '', value: '', enabled: true, type: 'text' }])
        }
      >
  ...
  return (
    <div className={rowClass}>
      ...
      {blank ? (
        ...
        <button className="kv-remove" title="Remove" onClick={() => remove(row.id)}>
          <X size={14} strokeWidth={2} />
        </button>
      )}
      {suggest.dropdown}
...
function AuthTab({ request }: { request: RequestNode }): React.JSX.Element {
  ...
  const helper: Record<typeof mode, string> = {
    ...
    apikey: 'The key-value pair will be added to your request as configured when you send it.',
