...
export function CommandPalette(): React.JSX.Element {
  ...
  useEffect(() => {
    let cancelled = false
    void window.relay.listRuns({ limit: 50 }).then((r) => {
  ...
  const commands = useMemo<CmdItem[]>(() => {
    ...
    return [
      ...
      { kind: 'command', id: 'go-runbook', label: 'Go to Runbook', run: go(() => ui.setView('runbook')) },
      { kind: 'command', id: 'go-history', label: 'Go to History', run: go(() => ui.setView('history')) },
      { kind: 'command', id: 'go-docs', label: 'Go to Docs', run: go(() => ui.setView('docs')) },
      { kind: 'command', id: 'env', label: 'Edit environments & variables', hint: '⌘E', run: go(() => ui.openEnvEditor()) },
      { kind: 'command', id: 'prefs', label: 'Preferences', hint: '⌘,', run: go(() => ui.openPrefs()) },
      { kind: 'command', id: 'shortcuts', label: 'Keyboard shortcuts', hint: '⌘/', run: go(() => ui.openShortcuts()) },
      { kind: 'command', id: 'import-openapi', label: 'Import OpenAPI…', run: () => { ui.closePalette(); void importOpenApi() } },
      { kind: 'command', id: 'import-postman', label: 'Import from Postman…', run: () => { ui.closePalette(); void importPostman() } }
    ]
            ...
            return (
              <div key={`${it.kind}-${it.id}`}>
                ...
                >
                  ...
                  {it.kind === 'request' && (
                    <>
                      <span className={`palette-m method-${it.method.toLowerCase()}`}>{it.method}</span>
                      <span className="palette-label">{it.label}</span>
                  ...
                  {it.kind === 'run' && (
                    <>
                      <span className={`palette-m method-${it.method.toLowerCase()}`}>{it.method}</span>
                      <span className="palette-label">{it.label}</span>
                      <span className={`palette-status ${it.error || (it.status ?? 0) >= 400 ? 'bad' : 'ok'}`}>
