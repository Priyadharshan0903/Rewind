...
export function CopyMenu({ req, disabled }: { req: RunRequest | null; disabled?: boolean }): React.JSX.Element {
  ...
  return (
    <div className="copy-menu-wrap">
      <button className="text-btn" disabled={disabled || !req} onClick={() => setOpen((v) => !v)}>
        ⧉ Copy as ▾
      </button>
      {open && (
        <>
